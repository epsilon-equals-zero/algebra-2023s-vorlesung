...
\begin{definition}
    Sei $R$ ein Integritätsbereichs-Ring. Dieser heißt \emph{euklidischer Ring}\index{Ring!euklidischer}, wenn es eine Funktion $H:R\setminus\{0\}\to\mathbb{N}$ mit
...
\begin{lemma}
    ...
    $$ p \mid f \cdot g \implies p \mid f \lor p \mid g. $$
\end{lemma}

\begin{proof}
    Wir zeigen die Aussage mittels Induktion nach $\deg fg = n+m$, wobei
    $$ f = \sum_{i=0}^n a_i x^i, g = \sum_{j=0}^n b_j x^j. $$

    Ist $n+m=0$, so sind also $f, g \in R$, womit folgt $p \mid f \lor p \mid g$, da $p$ prim in $R$ ist.

    Gilt $p \mid fg$, so folgt $p \mid a_n b_m$ und damit, da $p$ prim in $R$ ist, $p \mid a_n \lor p \mid b_m$. Nehmen wir \obda $p \mid a_n$ an. Schreiben wir nun $f = a_n x^n + f'$. Es gilt
    $$ fg = a_n x^n g + f' g. $$
    Nun teilt $p$ jedoch $fg, a_n, f' g$. Nach Induktionsvoraussetzung gilt damit $p \mid f' \lor p \mid g$, und damit $p \mid a_n, p \mid f'$ und damit $p \mid f$.
\end{proof}
...
\begin{lemma}
    ...
    Diese Darstellung ist eindeutig bis auf Äquivalenz, die Konstanten unterscheiden sich also lediglich um eine Einheit.
...
\begin{proof}
    ...
    wobei $b_i \in R$ und sich somit sofort die geforderte Darstellung ergibt.

    Seien nun $c_f \cdot f_0 = f = d \cdot g$ zwei Darstellungen. Schreiben wir
    $$ f_0 = \sum_{i=0}^\ell b_i x^i, g = \sum_{i=0}^\ell t_i x^i, $$
...
\begin{lemma}
    Sei $R$ faktoriell, $Q$ der Quotientenkörper von $R$. Sei $f \in R[x]$ irreduzibel in $R[x]$, $\deg f \geq 1$, so ist $f$ irreduzibel in $Q[x]$.
\end{lemma}
