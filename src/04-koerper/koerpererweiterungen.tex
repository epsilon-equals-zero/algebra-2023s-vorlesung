...
\begin{proof}
    ...
    Sei $d$ ein ggT von $f,\mu_\beta$ in $K(\delta)[x]$. Nach dem Lemma von Bezout existieren $p,q\in K(\delta)[x]$ mit $d=pf+q\mu_\beta$. Da der Polynomring $K(\delta)[x]$ die Voraussetzungen von \cref*{lemma:minimaliaet_ggt_als_linearkombination} erfüllt, hat $d$ minimalen Grad in $(\{f,\mu_\beta\})\triangleleft K(\delta)[x]$.
    Sei $\overline{d}$ ein ggT von $f,\mu_\beta$ in $\overline{K}[x]$. Die Analoge Argumentation liefert, dass $x-\beta$ minimalen Grad im von $f$ und $\mu_\beta$ erzeugten Ideal in $\overline{K}[x]$ haben muss. Da dieses Ideal klarerweise größer ist, folgt $\deg(x-\beta)\leq \deg(d)$ und wegen $d\mid x-\beta$ schließlich $d\sim (x-\beta)$. Wegen $-\beta=\ggT(f,\mu_\beta)(0)$ und $\alpha = \delta -c\beta$ gilt schließlich $\alpha,\beta\in K(\delta)$ und damit die Behauptung.
\end{proof}
