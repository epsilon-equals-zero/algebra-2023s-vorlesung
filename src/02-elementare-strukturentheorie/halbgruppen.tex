...
\begin{definition}
    ...
    \end{itemize}
    Ist $a'$ links- und rechtsinvers zu $a$ so nennt man $a'$ \emph{invers}\index{invers} zu $a$. Besitzt $a$ ein inverses Element, so heißt $a$ \emph{Einheit}\index{Einheit}.
\end{definition}
...
\begin{remark}
    ...
    Ist der Generator (die Variablenmenge) $X$ unendlich, so ist das erzeugte Monoid \emph{total frei}\index{Monoid!total frei} über $X$, das heißt es gelten genau die Gesetze, die in der Varietät gelten, wie man aus folgender Bemerkung erkennt.
\end{remark}
...
Wichtig für \cref{theorem:fundamentalsatz-der-arithmetik} ist die Menge der Primzahlen, welche wir mit dem Symbol $\mathbb{P}$ bezeichnen.
...
\begin{definition}
    ...
    \begin{itemize}
        \item $ a \cdot b = a \cdot b' \implies b = b', $ so heißt $a$ \emph{linkskürzbar}.\index{kürzbar!links-}
        \item $ b \cdot a = b' \cdot a \implies b = b', $ so heißt $a$ \emph{rechtskürzbar}.\index{kürzbar!rechts-}
        \item Ist $a$ links- und rechtskürzbar, so heißt $a$ \emph{kürzbar}.\index{kürzbar}
    \end{itemize}
\end{definition}
...
\begin{theorem}\label{theorem:monoid-gruppe-einbettung}
    ...
    \begin{enumerate}
        ...
        ist eine \emph{Einbettung}\index{Einbettung}, also ein injektiver Homomorphismus.
