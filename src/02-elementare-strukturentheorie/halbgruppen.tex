...
\begin{definition}
    ...
    \end{itemize}
    Ist $a'$ links- und rechtsinvers zu $a$ so nennt man $a'$ \emph{invers} zu $a$ und $a$ heißt dann \emph{Einheit}\index{Einheit}.
\end{definition}
...
\begin{proposition}\label{prop:halbgruppe-monoid-einbettung}
    ...
    $(H', \overline{\cdot}, e)$ ist dann ein Monoid und es gilt $\overline{\cdot}\vert_{H^2} = \cdot$.
\end{proposition}
...
\begin{theorem}[Fundamentalsatz der Arithmetik]\label{theorem:fundamentalsatz-der-arithmetik}
...
\begin{remark}
    ...
    Wir sagen, dass $a$ \emph{linkskürzbar} ist. Entsprechend definieren wir, dass $a$ \emph{rechtskürzbar} ist. \index{kürzbar!links-}\index{kürzbar!rechts-}
...
\begin{theorem}\label{theorem:monoid-gruppe-einbettung}
    ...
    \begin{figure}[H]
        ...
        \begin{tikzpicture}
            ...
            \node at (0.5, -4.2) {$\mathfrak{H^2} / \sim$};
        ...
        \end{tikzpicture}
        \caption{Visualisierung der Einbettung von $\mathfrak{H}$ in die Gruppen $\mathfrak{G}, \mathfrak{H}^2 / \sim$}
    \end{figure}
\end{theorem}
