...
\begin{example}
    ...
    Sei $M$ eine Menge und betrachte den Ring $(\mathcal{P}(M), \triangle, \emptyset, \id_{\mathcal{P}(M)}, \cap, M)$. Sei $A \subseteq M$ beliebig, so ist $\mathcal{P}(A) \vartriangleleft \mathcal{P}(M)$. Weiters kann $(\mathcal{P}(A), \triangle, \emptyset, \id_{\mathcal{P}(A)}, \cap, A)$ zu einem Ring mit 1 gemacht werden. Es handelt sich dabei um keinen Widerspruch zu \cref{remark:ideal-mit-1-trivial}, da hier ein anderes Einselement gefunden wird als im ursprünglichen Ring.
\end{example}
...
\begin{definition}
    Sei $R$ eine Ring, $I \vartriangleleft R$ ein Ideal und $\sim$ die wie in \cref{remark:ideal-congrel} vom Ideal induzierte Kongruenzrelation. Wir definieren den \emph{Faktorring} \index{Ring!Faktor-}
...
\begin{definition}
    ...
    Weiters nennen wir $R$ einen \emph{Hauptidealring}\index{Hauptideal!-ring}\index{Ring!Hauptideal-}, wenn gilt
...
\begin{proposition}\label{prop:frob_ringhomom}
    Sei $R$ ein kommutativer Ring mit 1, $\chara R = p \in \mathbb{P}$. Dann ist
...
\begin{theorem}\label{theorem:Quotientenkoerper}
    Sei $R$ ein kommutativer Ring mit $1 \neq 0$, der zusätzlich ein Integritätsbereich ist. Sei weiters $\sim \subseteq (R \times R^\times)^2$ wie in \cref{rem:ring-koerper} definiert. Dann gilt:
    \begin{enumerate}
...
\begin{proof}
    ...
    Daher existieren $\bar{\varphi}:L\to K$ mit $\bar{\varphi}\circ \varphi_0=\varphi$ und $\bar{\varphi_0}:K\to L$ mit $\bar{\varphi_0}\circ \varphi=\varphi_0$. Wir zeigen zuerst die folgenden Behauptung. Für jeden injektiven Homomorphismus
...
\begin{definition}\label{def:polynomring}
    Sei $R$ eine Ring mit $1$. Wir definieren den \emph{Polynomring über $R$}\index{Polynomring}
    ...
    Elemente von $R[x]$ bezeichnen wir als \emph{Polynome}\index{Polynom}.
