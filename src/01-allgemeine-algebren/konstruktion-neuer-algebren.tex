...
\begin{definition}\label{definition:unterstruktur}
    ...
    \end{itemize}
    Wir schreiben in diesem Fall $\mathfrak{S}\le \mathfrak{A}$. Ist $\mathcal{A}$ eine Gruppe (ein Monoid, ein Ring,...\footnote{Körper nehmen eine Sonderrolle ein, siehe dazu \cref{def:unterkoerper}}) so nennen wir $\mathcal{S}$ auch \emph{Unterring} (\emph{Untermonoid}, \emph{Unterring},...).
\end{definition}
...
\begin{example}
    ...
    Kongruenzrelationen auf $\mathfrak{A}$ gegeben. Diese nennt man auch \emph{triviale Kongruenzrelationen}\index{Kongruenzrelation!trivial}.
\end{example}
...
\begin{definition}
    Sei $\mathfrak{A}=(A,(f^\mathfrak{A}_i)_{i\in I})$ eine Algebra und sei $\sim\subseteq A^2$ eine Kongruenzrelation. Dann heißt $\mathfrak{A}/_\sim:=(A/_\sim,(f^{\mathfrak{A}/_\sim}_i)_{i\in I})$ \emph{Faktoralgebra}\index{Faktoralgebra} von $\mathfrak{A}$,
...
\begin{theorem}[Homomorphiesatz]\index{Homomorphiesatz}\label{satz:homomorphiesatz}
