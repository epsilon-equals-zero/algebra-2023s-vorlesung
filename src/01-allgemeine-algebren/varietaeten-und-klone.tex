...
\begin{example}
    ...
    Betrachtet man hingegen Gruppen über die Sprache $(+)$ wie in \Cref{rem:alternativegruppe}, so kann man die Gruppenaxiome nicht über Gesetze definieren.
\end{example}
...
\begin{definition}
    Sei $\mathfrak{A} = (A, (f_i)_{i \in I})$ eine Algebra und sei die Menge $\mathcal{T}^{(n)}(\mathfrak{A}) := \{f: A^n \to A\mid f\;\text{ist Termfunktion von}\;\mathfrak{A}\}$. Es ist dann $\mathcal{T}(\mathfrak{A}) := \bigcup_{n \ge 1} \mathcal{T}^{(n)}(\mathfrak{A})$ ein Klon und wird \emph{Termklon von $\mathfrak{A}$}\index{Termklon} genannt.
\end{definition}