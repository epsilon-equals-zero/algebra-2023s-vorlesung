...
\begin{definition}\label{def:algebra}
    Seien $A$ eine beliebige Menge, $\tau = (n_i)_{i \in I}$ eine Familie aus $\mathbb{N}_0$ über eine beliebige Indexmenge $I$ und $(f_i)_{i \in I}$ eine Familie von Funktionen, wobei $f_i: A^{n_i} \to A$ ist. 
     Das Tupel $\mathfrak{A} = (A, (f_i)_{i \in I})$ heißt dann \emph{(allgemeine) Algebra}\index{Algebra!allgemeine} vom \emph{Typ}\index{Algebra!Typ} $\tau$. Die einzelnen Funktionen $f_i$ haben die \emph{Stelligkeit}\index{Stelligkeit} oder \emph{Arität}\index{Arität} $n_i$.
\end{definition}
