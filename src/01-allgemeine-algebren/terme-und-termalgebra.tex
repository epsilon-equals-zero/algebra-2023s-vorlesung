...
\begin{definition}
    ...
    Für einen Term $t \in T$ heißt $\lvl(t) := \min\{k \mid t \in T_k\}$ \emph{Stufe von $t$}\index{Term!Stufe}. 
    
    Weiter werden die \emph{Variablen eines Terms}\index{Term!Variablen} rekursiv definiert. Für $x \in X$ ist $\var(x) := \{x\}$ und für $t = f_i(t_1, \ldots, t_n)$ weiter $\var(t) := \bigcup_{j \in \{1, \ldots, n_i\}} \var(t_j)$.
\end{definition}
...
\begin{definition}
    Sei $t, s \in T$ Terme über eine Sprache $(f_i)_{i \in I}$, dann heißt $t \approx s$ \emph{Gesetz}\index{Gesetz}. Ein Gesetz kann auch als Paar $(t, s)$ von zwei Termen gesehen werden.
