...
\begin{definition}
    ...
    Ein Element $t \in T$ heißt \emph{Term}\index{Term}, die Elemente aus $X$ \emph{Variablen}\index{Variable}, $(f_i)_{i \in I}$ \emph{Sprache}\index{Sprache} und die Menge $T$ beschreibt alle \emph{Terme über $(X, (f_i)_{i \in I})$}.
    Für einen Term $t \in T$ heißt $\lvl(t) := \min\{k \mid t \in T_k\}$ die \emph{Stufe von $t$}\index{Term!Stufe}. 
    
    Weiter werden die \emph{Variablen eines Terms}\index{Term!Variablen} rekursiv definiert. Für $x \in X$ ist $\var(x) := \{x\}$ und für $t = f_i(t_1, \ldots, t_n)$ ist $\var(t) := \bigcup_{j \in \{1, \ldots, n_i\}} \var(t_j)$.
\end{definition}

\begin{example}
    Seien $X = \{x,y,z\}$ und $(f_1, f_2, f_3) = (+, \cdot, -)$ mit Aritäten $(2,2,1)$. Damit erhält $x,y,z$ als Terme 0-ter Stufe, $-x, x+x, x\cdot z, z + x, \ldots$ als Terme 1-ter Stufe, $(-x) + y, (x \cdot z) - y, \ldots$ als Terme 2-ter Stufe etc.
\end{example}

\begin{definition}
    Sei $T$ die Menge aller Terme über $(X, (f_i)_{i \in I})$. Es ist dann $\mathfrak{T}(X, (f_i)_{i \in I}) := (T, (f_i^\mathfrak{T}))$ die \emph{(erzeugte) Termalgebra}\index{Termalgebra} eine Algebra vom Typ $\tau = (n_i)_{i \in I}$, wobei $f_i^\mathfrak{T}: T^{n_i} \to T, (t_1, \ldots, t_{n_i}) \mapsto f_i(t_1, \ldots, t_n)$.
\end{definition}
...
\begin{definition}
    Seien $t, s \in T$ Terme über einer Sprache $(f_i)_{i \in I}$, dann heißt $t \approx s$ \emph{Gesetz}\index{Gesetz}. Ein Gesetz kann auch als Paar $(t, s)$ von zwei Termen gesehen werden.

    Sei $\mathfrak{A} = (A, (f_i^\mathfrak{A})_{i \in I})$ eine Algebra über derselben Sprache, dann erfüllt \emph{$\mathfrak{A}$ das Gesetz $t \approx s$} oder kurz \emph{$\mathfrak{A} \models t \approx s$}, wenn 
    $$ \forall (\alpha: \var(t) \cup \var(s) \to A): \overline{\alpha}(t) = \overline{\alpha}(s), $$
    oder anders formuliert, wenn die Termfunktionen $t^\mathfrak{A}$ und $s^\mathfrak{A}$ übereinstimmen.
\end{definition}