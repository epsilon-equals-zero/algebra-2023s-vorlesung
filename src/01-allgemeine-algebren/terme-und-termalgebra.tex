...
\begin{definition}
    ...
    Weiter werden die \emph{Variablen eines Terms}\index{Term!Variablen} rekursiv definiert. Für $x \in X$ ist $\var(x) := \{x\}$ und für $t = f_i(t_1, \ldots, t_{n_i})$ ist $\var(t) := \bigcup_{j \in \{1, \ldots, n_i\}} \var(t_j)$.
\end{definition}
...
\begin{definition}
    Sei $T$ die Menge aller Terme über $(X, (f_i)_{i \in I})$. Es ist dann $\mathfrak{T}(X, (f_i)_{i \in I}) := (T, (f_i^\mathfrak{T}))$, die \emph{(erzeugte) Termalgebra}\index{Termalgebra}, eine Algebra vom Typ $\tau = (n_i)_{i \in I}$, wobei $f_i^\mathfrak{T}: T^{n_i} \to T, (t_1, \ldots, t_{n_i}) \mapsto f_i(t_1, \ldots, t_{n_i})$.
\end{definition}
